...
export const Abilities: import('../sim/dex-abilities').AbilityDataTable = {
	...
	samurai: { // combines [Moxie] + [Defiant]
	...
	secretagent2: { // REFLECT TYPE but as an ability = pokemon.side.allySide?.pokemon
		onFoeSwitchIn(source) {
			if (this.effectState.target !== source.side.foe.active[source.side.foe.active.length - 1 - source.position]) return;
			const target = source.side.foe.active[source.side.foe.active.length - 1 - source.position];
			const adjacentFoe = source.adjacentFoes()[0]; 
		...
		onSwitchIn(pokemon) {
			if (this.effectState.target !== pokemon.side.foe.active[pokemon.side.foe.active.length - 1 - pokemon.position || pokemon.side.foe.active.length - 2 - pokemon.position]) return;
			const foe = pokemon.side.foe.active[pokemon.side.active.length - 1 - pokemon.position || pokemon.side.active.length - 2 - pokemon.position]
            const adjacentFoe = pokemon.adjacentFoes()[0]; 
            const oldApparentType = pokemon.apparentType;
            if (!foe || foe.fainted || foe.newlySwitched) return false;
	...
	yabairys: { // [STORM DRAIN] but for SOUND MOVES and heals instead of boosted stats
		onFoeTryMove(pokemon, target, move) {
	...
	mothernature: { // combines [Aroma Veil] + [Healer] but 100% chance to heal status end of evey turn 
		...
		onSetStatus(status, target, source, effect) {
			if (status.id !== 'psn' && status.id !== 'tox' && status.id !== 'par' && status.id !== 'slp' && status.id !== 'brn' && status.id !== 'frz') return;
			if ((effect as Move)?.status) {
				this.add('-immune', target, '[from] ability: Mother Nature');
			} 
			return false;
		},
		onResidualOrder: 5,
	...
	nohmask: { // a copy of DISGUISE
		onDamage(damage, target, source, effect) {
			if (effect && effect.effectType === 'Move' &&
				...
				this.add("-activate", target, "ability: Noh Mask");
	...
	publicitystunt: { // SOLAR POWER but with Water type instead + WATER ABSORB
		onTryHit(target, source, move) {
			if (target !== source && move.type === 'Water') {
				if (!this.heal(target.baseMaxhp / 4, target, target)) {
					this.add('-immune', target, '[from] ability: Publicity Stunt');
				}
				return null;
			}
		},
	...
	screencleaner: {
		onStart(pokemon) {
			let activated = false;
			for (const sideCondition of ['reflect', 'lightscreen', 'auroraveil', 'infirmary', 'hologram']) {
				for (const side of [pokemon.side, ...pokemon.side.foeSidesWithConditions()]) {
					if (side.getSideCondition(sideCondition)) {
						if (!activated) {
