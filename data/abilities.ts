...
export const Abilities: import('../sim/dex-abilities').AbilityDataTable = {
	...
	nouturn: {
		onFoeTryMove(target, source, move) {
			...
			if (move.target === 'foeSide' || (move.target === 'all' && move.id === 'teleport' && move.id === 'batonpass' && move.id === 'rainshaman' && move.id === 'chillyreception' && !targetAllExceptions.includes(move.id))) {
				return;
			}
