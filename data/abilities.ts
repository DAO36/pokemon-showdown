...
export const Abilities: import('../sim/dex-abilities').AbilityDataTable = {
	...
	yandere: {
		onFoeTryMove(source, pokemon, move) {
			...
			const yandereHolder = this.effectState.pokemon;
			if (move.id === 'teleport' || move.id === 'batonpass') {
				this.attrLastMove('[still]');
				this.add('cant', yandereHolder, 'ability: Yandere', move, '[from] ' + pokemon);
				return false;
			}
		},
		flags: {breakable: 1},
	...
	ere: {
		onFoeTryMove(pokemon, target, move) {
			...
			if (move.id === 'teleport' || move.id === 'batonpass') {
				this.attrLastMove('[still]');
				this.add('cant', yandereHolder, 'ability: Yandere', move, '[from] ' + yandereHolder);
				return false;
			}
		},
		flags: {breakable: 1},
