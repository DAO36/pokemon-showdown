...
export class BattleActions {
	...
	hitStepAccuracy(targets: Pokemon[], pokemon: Pokemon, move: ActiveMove) {
		...
		for (const [i, target] of targets.entries()) {
			...
			if (accuracy !== true && !this.battle.randomChance(accuracy, 100)) {
				...
				if (!move.ohko && pokemon.hasAbility('investment')) {
					this.battle.add('-activate', pokemon, 'ability: Investment');
					this.battle.boost({ spe: 2 }, pokemon);
				}
				hitResults[i] = false;
				continue;
		...
		if (move.breaksProtect) {
			for (const target of targets) {
				...
				for (const effectid of [
					'banefulbunker', 'burningbulwark', 'lunarshield', 'phoenixshield', 'kingsshield', 'obstruct', 'protect', 'silktrap', 'spikyshield',
				]) {
	...
	modifyDamage(
		...
		if (pokemon.status === 'brn' && move.category === 'Physical' && !pokemon.hasAbility('guts') && !pokemon.hasAbility('highonasacoco')) {
